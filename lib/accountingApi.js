 ...
 *
 * Some panels need a per-day answer that the API only gives one day at a time
 * (distinct counts, which have no bulk form). Firing thirty of those at once
 * is rude to a shared ClickHouse; doing them serially is slow. Results stay in
 * input order.
 */
export async function mapWithConcurrency(items, fn, limit = 8) {
